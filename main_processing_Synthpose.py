...
for subject_folder in root_folder_marker_less.iterdir():
    ...
    # 2. Create model
    # ------------------------------------------------------------
    model_name = folder_to_save_ml / "markerless" / subject_folder.name / "markerless_model"
    ...
    # ------------------------------------------------------------
    model_name = folder_to_save_ml / "markerless" / subject_folder.name / "markerless_model.bioMod"
    for task_file in subject_folder.glob("*.c3d"):
        ...
        file_exported = folder_to_save_ml / "markerless" / subject_folder.name / f"{task_file.stem}.csv"
        # check if the folder of the output file exist, if not create it
...
for subject_folder in root_folder_marker_based.iterdir():
    ...
    # 2. Create model
    # ------------------------------------------------------------
    model_name = folder_to_save_mb / "marker_based" / subject_folder.name / "markerbased_model"
    # check if the folder of the output file exist, if not create it
    if not model_name.parent.exists():
        model_name.parent.mkdir(parents=True, exist_ok=True)
    print("⚙ Creating model...")
    ...
    # 3. Process all task trials
    # ------------------------------------------------------------
    model_name = folder_to_save_mb / "marker_based" / subject_folder.name / "markerbased_model.bioMod"
    # check if the folder of the output file exist, if not create it
    if not model_name.parent.exists():
        model_name.parent.mkdir(parents=True, exist_ok=True)
    for task_file in subject_folder.glob("*.c3d"):
        ...
        print(f"⚙ Processing task: {task_file.name}")
        file_exported = folder_to_save_mb / "marker_based" / subject_folder.name / f"{task_file.stem}.csv"
        # check if the folder of the output file exist, if not create it
        if not file_exported.parent.exists():
            file_exported.parent.mkdir(parents=True, exist_ok=True)
        kinematics.main(
