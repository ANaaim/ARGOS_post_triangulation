...
path_to_data = Path(
    r".\data\pre_processed\absolute_position_errors.parquet"
)
...
def plot_error_quantiles(
    ...
    for component in components:
        ...
        for model_index, model in enumerate(available_models):
            ...
            offset = (
                model_index - (len(available_models) - 1) / 2
            ) * 0.25

            for point_index, point in enumerate(available_points):

                q = q_model.filter(
                    pl.col("point_short") == point
                )
    ...
    points_in_data = set(
                quantiles
                .get_column("point_short")
                .unique()
                .to_list()
                )
    available_points = [
                point for point in POINT_ORDER
                if point in points_in_data
                ]
